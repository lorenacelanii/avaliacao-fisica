...
document.addEventListener('click', (e) => {
  if (e.target.classList.contains('editBtn')) {
    ...
    const novoValor = prompt(`Editar ${arr[idx].nome} (${unidade})`, valorAntigo);
    if (novoValor === null) return;
    const num = parseFloat(novoValor);
    if (isNaN(num)) return alert('Valor inválido');
    arr[idx].medida = num;
    tipo === 'dobra' ? (atualizarListaDobras(), calcularGordura()) : atualizarListaPerims();
  }
});
...
function calcularGordura() {
  const obrig = ['Peitoral', 'Axilar média', 'Tríceps', 'Subescapular', 'Abdominal', 'Coxa medial', 'Supra-ilíaca'];
  if (!obrig.every(n => dobras.some(d => d.nome === n))) return;
  const soma = dobras.filter(d => obrig.includes(d.nome)).reduce((acc, d) => acc + d.medida, 0);
  const idade = parseInt(idadeInput.value);
  const sexo = document.getElementById('sexo').value;
  if (!idade || !sexo) return;
  const dens = sexo === 'masculino'
  ...
  document.getElementById('percentualGordura').textContent = `% de Gordura Corporal: ${perc.toFixed(2)}%`;
}
...
document.getElementById('btnExportar').addEventListener('click', () => {
  ...
  addText('PA Sistólica', document.getElementById('paSistolica').value + ' mmHg');
  addText('PA Diastólica', document.getElementById('paDiastolica').value + ' mmHg');
